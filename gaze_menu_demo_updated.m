...
%% Config
nRegions      = 4;                 % 2x2 grid
nSubPoints    = 4;                  % calibration sub-points PER region (see note below)
framesPerSub  = 6;                  % calibration frames per sub-point (total/region = nSubPoints*framesPerSub)
...
liveDetectEveryN   = 5;   % re-run FULL face detection every N frames (calibration AND live);
                           % other frames reuse the cached bbox ("tracking") — same pipeline
                           % is used in both phases now, on purpose (see note below)
liveDownsample     = 0.5; % downsample factor applied before face detection
cameraDisplayScale = 0.4; % downsample factor for the background feed image (display only)

% NOTE on nSubPoints: a single calibration dot per region only captures
% where a user's gaze template sits when fixating one exact pixel — but
% at inference they'll look anywhere within the region's box, not that
% exact pixel. Sampling nSubPoints spread across each region captures
% that natural spread, and inference classifies against whichever
% sub-template is nearest (not one blended-away mean), which handles
% within-region variation better than a single centroid per region.
%
% NOTE on liveDetectEveryN unification: calibration and live inference
% now go through the EXACT SAME tracked-detection code path
% (extractEyeCropTracked). Previously calibration re-detected the face
% fresh every frame while live inference mostly reused a cached/
% downsampled bbox — that mismatch between how training crops and
% inference crops were produced is a common, easy-to-miss source of
% "the model is suddenly very inaccurate," independent of anything
% about calibration point placement.
...
targetSize = [30 60];
boxW = scrW/2 * 0.8; boxH = scrH/2 * 0.8; % region box size, used for both calibration sub-point placement and live UI

%% --- Calibration phase: nSubPoints per region, tracked detection (matches live pipeline) ---
fig = figure('Units','pixels','Position',[1 1 scrW scrH], ...
             'MenuBar','none','ToolBar','none','Color','k');

% Sub-points arranged inset within each region's box (roughly its 4
% quadrants), not at the exact box edges, so they stay clearly "inside"
% that region and don't get confused with a neighboring one.
subInsetX = boxW * 0.18;
subInsetY = boxH * 0.18;
subOffsets = [-subInsetX -subInsetY; subInsetX -subInsetY; -subInsetX subInsetY; subInsetX subInsetY];
if nSubPoints ~= size(subOffsets,1)
    error('This demo''s sub-point layout is hardcoded for nSubPoints = 4.');
end

classVecs = cell(nRegions, nSubPoints);
subScreenXY = zeros(nRegions*nSubPoints, 2);
subRegionIdx = zeros(nRegions*nSubPoints, 1);
calBBoxCache = [];   % same tracked-detection cache mechanism as the live loop
calFrameCounter = 0;
rowIdx = 0;
...
for p = 1:nRegions
    ...
    for s = 1:nSubPoints
        rowIdx = rowIdx + 1;
        sx = cx + subOffsets(s,1);
        sy = cy + subOffsets(s,2);
        subScreenXY(rowIdx,:) = [sx, sy];
        subRegionIdx(rowIdx) = p;

        clf(fig);
        axc = axes('Parent', fig, 'Position', [0 0 1 1], 'Color', 'k');
        hold(axc, 'on'); axis(axc, [0 scrW 0 scrH]); axis(axc, 'off');
        plot(axc, sx, sy, 'r.', 'MarkerSize', 50);
        title(axc, sprintf('Calibrating "%s" (%d/%d) — point %d/%d', ...
              labels{p}, p, nRegions, s, nSubPoints), 'Color', 'w', 'FontSize', 16);
        drawnow;
        pause(0.6);

        vecs = [];
        for f = 1:framesPerSub
            frame = snapshot(cam);
            calFrameCounter = calFrameCounter + 1;
            if mod(calFrameCounter, liveDetectEveryN) == 0
                calBBoxCache = []; % periodic fresh detection, identical policy to live loop
            end
            [crop, calBBoxCache] = extractEyeCropTracked(frame, faceDetector, eyeDetector, ...
                calBBoxCache, liveDownsample);
        ...
        if isempty(vecs)
            close(fig); clear cam;
            error('No valid eye crops for region "%s" sub-point %d. Check lighting/webcam position.', labels{p}, s);
        end
        classVecs{p,s} = vecs;
    end
end

subMeans = zeros(nRegions*nSubPoints, size(classVecs{1,1},2));
rowIdx = 0;
for p = 1:nRegions
    for s = 1:nSubPoints
        rowIdx = rowIdx + 1;
        subMeans(rowIdx,:) = mean(classVecs{p,s}, 1);
    end
end
...
while ishandle(fig)
    ...
    if isempty(v)
        ...
        % Distance to all nRegions*nSubPoints sub-templates.
        d = vecnorm(subMeans - v, 2, 2);

        % --- Aggregation: for each region, take its NEAREST sub-template
        % (not an average) — this is the "aggregate during inference" step.
        % Using min rather than mean lets each region's decision boundary
        % flex around however many distinct sub-templates it has, instead
        % of collapsing them into one blurred centroid.
        regionDist = inf(nRegions, 1);
        for p = 1:nRegions
            regionDist(p) = min(d(subRegionIdx == p));
        end
        [~, region] = min(regionDist);

        % --- Continuous cursor position: inverse-distance weighted blend
        % across ALL sub-template positions (finer-grained than blending
        % just 4 region centers — effectively nRegions*nSubPoints anchor
        % points now), so the cursor glides smoothly.
        w = 1 ./ (d + 1e-6);
        w = w / sum(w);
        targetX = sum(w .* subScreenXY(:,1));
        targetY = sum(w .* subScreenXY(:,2));
